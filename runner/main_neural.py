...
import neural_tagger as tagger
import neural_parser as parser

print("Running neural baseline system...")

tagger1 = tagger.train_neural(baseline_tagger.train_data[:6000], n_epochs=1)
parser1 = parser.train_neural(baseline_parser.train_data[:6000], n_epochs=1)

# If using other data, make sure to use projectivize.py
input_parser = parser.dev_data
input_tagger = tagger.dev_data

with open("output_neural.conllu", "w") as f:
    for elem in input_tagger:
        sen = ['<root>']
        for word in elem:
            sen.append(word[0])
        pred_tags = tagger1.predict(sen)
        pred_tags.insert(0, '<root>')
